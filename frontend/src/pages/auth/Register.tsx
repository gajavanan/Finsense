...
const schema=z.object({ name: z.string().min(2), email: z.string().email(), phone: z.string().optional(), password: z.string().min(6), confirm: z.string().min(6)}).refine(d=>d.password===d.confirm, {message:'Passwords do not match', path:['confirm']})
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-sky-50 to-violet-50 dark:from-slate-950 dark:to-slate-900">
      <form onSubmit={handleSubmit(onSubmit)} className="card w-full max-w-md space-y-3">
        ...
        <div><label className="text-sm font-medium">Full name</label><input {...register('name')} className="input mt-1" placeholder="Alex Morgan"/><p className="text-xs text-red-500">{errors.name?.message as any}</p></div>
        <div><label className="text-sm font-medium">Email</label><input {...register('email')} className="input mt-1" placeholder="alex@example.com"/><p className="text-xs text-red-500">{errors.email?.message as any}</p></div>
        <div><label className="text-sm font-medium">Phone (Optional)</label><input {...register('phone')} className="input mt-1" placeholder="+1 (555) 000-0000"/><p className="text-xs text-red-500">{errors.phone?.message as any}</p></div>
        ...
        <div><label className="text-sm font-medium">Confirm password</label><input type="password" {...register('confirm')} className="input mt-1" placeholder="••••••••"/><p className="text-xs text-red-500">{errors.confirm?.message as any}</p></div>
