...
api.interceptors.response.use(r=>r, err=>{
  ...
  if(status===403){
    const isNotVerified = detailCode === 'EMAIL_NOT_VERIFIED' || (typeof detailStr === 'string' && detailStr.includes('Email not verified')) || (typeof detailStr === 'string' && detailStr.includes('verify'))
    if(isNotVerified){
  ...
  if(!err.response){
    const e:any = new Error('Unable to connect to FinSense server. Check backend is running on http://localhost:8000')
    e.status=0
